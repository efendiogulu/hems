...
def User_Convenience_calc(stop):
    ...
    constraint_starter = numpy.zeros((T_rh), dtype = int) #numpy.zeros(129)
    vehicle_charge_deadline = 0
...
for rh in range(0, nsteps):
    ...
    availability_difference =  difference(availability_full[0+shift:T_rh-1+shift]) #T_rh-1 = 128
    P_pv = P_pv_full[0+shift:T_rh+shift] #kW to W
    P_demand = P_demand_full[0+shift:T_rh+shift] #kW to W
...
#        model.energy = Objective(expr = sum(0.99*(model.P_B_ch[j]*delta_t - model.P_B_dis[j]*delta_t +model.P_EV_ch[j]*delta_t - model.P_EV_dis[j]*delta_t)  + 0.01*User_Convenience_exp[j]  for j in model.RH), sense = minimize)
    if objective_function == 'energy':
        ...
        model.cost = Objective(expr = sum(model.P_grid[j]*c_buy*delta_t + User_Convenience_exp[j]  for j in model.RH), sense = minimize)
        ...
        model.income = Objective(expr = sum(model.P_grid[j]*c_buy*delta_t-model.P_inject[j]*c_sell*delta_t + User_Convenience_exp[j]  for j in model.RH), sense = minimize)
        ...
        model.income = Objective(expr = sum(model.P_inject[j]*c_sell*delta_t + User_Convenience_exp[j]  for j in model.RH), sense = maximize)
#    elif objective_function == 'no_v2h':
#        model.no_v2h = Objective(expr = sum(-model.P_grid[j]*c_buy*delta_t + User_Convenience_exp[j]  for j in model.RH), sense = minimize)    
...
total_EV_drive = (len(availability_full)-sum(availability_full))*P_EV_drive*delta_t/1000
...
total_cost_energy = sum(P_grid_sol)*delta_t*c_buy
total_income_energy = sum(P_inject_sol)*delta_t*c_sell
total_profit_energy = (-sum(P_grid_sol)*delta_t*c_buy + sum(P_inject_sol)*delta_t * c_sell)
...
#Grid + Inject
    ...
    plt.gcf().autofmt_xdate()
    plt.title('Grid and Inject Power for '+ objective_function)
    plt.legend()
    plt.xlabel('Opt. Periods(15min)')
    plt.ylabel('Power(W)')
    plt.tight_layout()
...
#Savings
    fig_P_grid_price = plt.plot(time[0+shift_week:one_week+shift_week], -1*P_grid_sol[0+shift_week:one_week+shift_week]*c_buy*delta_t, linewidth=width, label = 'Cost')
    fig_P_inject_price = plt.plot(time[0+shift_week:one_week+shift_week], P_inject_sol[0+shift_week:one_week+shift_week]*c_sell*delta_t, linewidth=width, label = 'Earnings')
